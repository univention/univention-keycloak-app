...
| oidc | object | `{"rp":{"umcserver":{"existingSecret":{"keyMapping":{"password":"password"},"name":"{{ printf \"%s-oidc-rp-umc-server-credentials\" .Release.Name }}"},"password":null}}}` | Configuration for Open ID Connect Clients |
